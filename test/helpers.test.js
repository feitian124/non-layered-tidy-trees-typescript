...
test('Layout class', () => {
    ...
    expect(boundingBox).toEqual(expect.objectContaining({ left: 5, right: 175, top: 0, bottom: 50 }));
});
...
test('Big root, small child', () => {
    ...
    expect(boundingBox).toEqual(expect.objectContaining({ left: -25, right: 75, top: 0, bottom: 100 }));
});
...
describe('Layout.getSize', () => {
    test('big root, small child', () => {
        ...
        expect(bb).toEqual(expect.objectContaining({ left: -25, right: 75, top: 0, bottom: 100 }));
    });
    ...
    test('small root, big child', () => {
        ...
        l.layout(t);
        const bb = l.getSize(t);
        expect(bb).toEqual(expect.objectContaining({ left: 10, right: 110, top: 0, bottom: 120 }));
    });
});
